...
void ModbusBLVmotor::setM1Speed(int speed)
{
  ...
  sendModbusPacket(M1ADR, SINGLEWRITE, REG_RotationSpeed, (uint16_t)speedRPM);
  Serial.print(">> M1, M2 (rpm)= "); Serial.print(speedRPM);
   
  if (reverse)
  {
    sendModbusPacket(M1ADR, SINGLEWRITE, REG_DriveMotor, CMD_Backward);
  }
  else
  {
    sendModbusPacket(M1ADR, SINGLEWRITE, REG_DriveMotor, CMD_Forward);
  }
}
...
void ModbusBLVmotor::setM2Speed(int speed)
{
  ...
  //set rotation speed to register 
  sendModbusPacket(M2ADR, SINGLEWRITE, REG_RotationSpeed, (uint16_t)speedRPM);
  Serial.print(", "); Serial.println(speedRPM);
   
  if (reverse)
  {
    sendModbusPacket(M2ADR, SINGLEWRITE, REG_DriveMotor, CMD_Backward);
  }
  else
  {
    sendModbusPacket(M2ADR, SINGLEWRITE, REG_DriveMotor, CMD_Forward);
  }
}
